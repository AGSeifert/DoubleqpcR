...
#' You need to have input.cq and input.raw and input.raw.meld from "read.fluorescenceTable" / "read.cqTable" or build them manuaally.
#'
#' !This will fail sometimes, do to uniroot() function returning 0!
#'
#' @import drc
#' @param cq.new The Column name of the new Cq values.
#' @param method The method used to calculate the Cq value. "TP" for Turning Point (first derivative) . "SD" for first expnential incline (second derivative).
...
#' @export
calcCq <- function(method = "TP", fct = "ll5", cq.new = "calcCq"){
